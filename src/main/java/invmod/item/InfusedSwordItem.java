...
public class InfusedSwordItem extends SwordItem {
    public InfusedSwordItem(Properties props) {
        super(ModToolTiers.INFUSED_GOLD, props.attributes(SwordItem.createAttributes(Tiers.GOLD, 4, -2.4F)));
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        ...
        if (player.isCrouching()) {
            ...
            if (level instanceof ServerLevel sw) {
                sw.addParticle(ParticleTypes.HEART, player.getX() + 1.5D, player.getEyeY(), player.getZ(), 1, 0, 0);
                sw.addParticle(ParticleTypes.HEART, player.getX() - 1.5D, player.getEyeY(), player.getZ(), 1, 0, 0);
                sw.addParticle(ParticleTypes.HEART, player.getX(), player.getEyeY(), player.getZ() + 1.5D, 1, 0, 0);
                sw.addParticle(ParticleTypes.HEART, player.getX(), player.getEyeY(), player.getZ() - 1.5D, 1, 0, 0);
            }
